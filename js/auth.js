if (typeof SUPABASE_URL === 'undefined') {
    ...
    var SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlhenpneGFjZHdoYWh1am94dGF6Iiwic2VjcmV0IjoiIn0';
}
...
async function checkRegistrationEmail(email) {
    ...
    try {
        var blocked = await supabaseQuery(
            'email_domains?type=eq.block&select=domain'
        );

        blocked = blocked || [];

        for (var i = 0; i < blocked.length; i++) {
            var blockedValue = String(blocked[i].domain || '')
                .trim()
                .toLowerCase();

            if (!blockedValue) continue;

            if (blockedValue.charAt(0) === '@') {
                blockedValue = blockedValue.substring(1);
            }

            if (
                blockedValue.includes('@') &&
                blockedValue === normalizedEmail
            ) {
                return {
                    allowed: false,
                    reason: 'Цей Email заборонений для реєстрації'
                };
            }
            ...
            if (
                ...
                return {
                    allowed: false,
                    reason: 'Реєстрація з доменом @' + domain + ' заборонена'
                };
            }
        }
        ...
        console.error('Email blocklist check failed:', error);

        return {
            allowed: false,
            reason: 'Не вдалося перевірити можливість реєстрації. Спробуйте ще раз.'
        };
    }
}

async function isRegistrationEmailAllowed(email) {
    var result = await checkRegistrationEmail(email);
...
async function syncProfile(allowCreate) {
    ...
    var rows = await supabaseQuery(
        'users?id=eq.' + session.user.id + '&select=*'
    );
    ...
    if (!profile && allowCreate) {
        ...
        try {
            ...
            var retryRows = await supabaseQuery(
                'users?id=eq.' + session.user.id + '&select=*'
            );

            profile = retryRows && retryRows.length > 0
                ? retryRows[0]
                : null;
...
async function loginUser(email, password, fullName) {
    try {
        ...
        if (error) {
            if (
                error.message &&
                error.message.toLowerCase().indexOf('invalid') !== -1
            ) {
        ...
        if (fullName !== undefined) {
            ...
            if (!enteredName || enteredName !== storedName) {
                ...
                throw new Error(
                    'Ім\'я та прізвище не збігаються з даними акаунта'
                );
            }
        }
...
async function registerUser(
    ...
    try {
        ...
        if (
            ...
            throw new Error(
                'Реєстрацію нових користувачів тимчасово вимкнено адміністрацією'
            );
        }
        ...
        if (!emailCheck.allowed) {
            throw new Error(
                emailCheck.reason ||
                'Реєстрація з цим Email заборонена'
            );
        }
        ...
        if (error) {
            if (
                error.message &&
                error.message.toLowerCase().indexOf('already') !== -1
            ) {
                throw new Error(
...
async function recordUserSession(
    ...
    try {
        var existing =
            await supabaseQuery(
                'user_sessions?session_token=eq.' +
...
async function verifyRegistrationOtp(
    ...
    try {
        ...
        var { data, error } =
            await window.sb.auth.verifyOtp({
                email: email,
                token: code,
                type: 'signup'
            });
        ...
        if (error) {
            if (
                ...
                throw new Error(
                    'Код прострочено. Натисніть "Надіслати код ще раз"'
                );
            }
        ...
        if (
            ...
            throw new Error(
                'Не вдалося підтвердити код. Спробуйте ще раз'
            );
        }
        ...
        if (profile) {
            ...
            if (
                ...
                try {
                    ...
                    console.warn(
                        'Failed to backfill profile extras:',
                        e
                    );
                }
            }
...
async function verifyPasswordResetOtp(
    ...
    if (error) {
        if (
            ...
            throw new Error(
                'Код прострочено. Запросіть новий'
            );
        }
