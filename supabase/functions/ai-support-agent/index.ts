...
${targetBlock}
ТИ АГЕНТ ПІДТРИМКИ. Спочатку визнач, чи цей тикет можна вирішити ОДРАЗУ (проста відповідь по системі, без потреби перевіряти людину/логи/IP/докази), чи потрібне РОЗСЛІДУВАННЯ (скарги на когось, підозра на порушення, багоюз, бан на 5+ днів тощо).
  ...
  "reason": "коротка причина"
    ...
    const groqResponse = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "Ти агент підтримки. Відповідай тільки JSON." },
